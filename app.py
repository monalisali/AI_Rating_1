...
def process_single_question(row_num, question, reference_answer, enable_scoring, scoring_prompt_template=None):
    """处理单个问题：获取AI回答 + 评分"""
    try:
...
def process_questions():
    ...
    def worker(row_num, question, reference_answer):
        result = process_single_question(row_num, question, reference_answer, enable_scoring, scoring_prompt_template)
    ...
    def generate():
        ...
        yield f"data: {json.dumps({'type': 'complete', 'output_filename': output_filename_xlsx, 'output_filename_html': output_filename_html}, ensure_ascii=False)}\n\n"
